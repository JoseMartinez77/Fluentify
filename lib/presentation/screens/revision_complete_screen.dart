...
const String apiUrl = 'https://api.textgears.com/analyze';

class RevisionCompleteScreen extends StatefulWidget {
...
class _RevisionCompleteScreenState extends State<RevisionCompleteScreen> {
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      _analyzeText(_textEditingController.text);
                    },
  ...
  Future<void> _analyzeText(String text) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (result['status'] == true) {
        ...
        // Mostrar errores gramaticales y de ortografía
        analysisResultTemp +=
            _extractGrammarAndSpellingErrors(analysisResponse);
  ...
  String _extractGrammarAndSpellingErrors(
      Map<String, dynamic> analysisResponse) {
    if (analysisResponse.containsKey('grammar') &&
        analysisResponse['grammar'].containsKey('errors')) {
      final grammarErrors = analysisResponse['grammar']['errors'];
      const grammarErrorText = 'Errores gramaticales y de ortografía:\n';

      // Variable para llevar el contador de errores
      int errorCount = 1;

      return grammarErrors.fold(grammarErrorText, (previousValue, error) {
        // Eliminar la parte {en:} de la descripción si existe y el punto al final
        final description = error['description']
            .toString()
            .replaceAll(RegExp(r'\{[a-z]{2}:'), '')
            .replaceAll('}', '')
            .replaceAll(
                RegExp(r'\.$'), ''); // Eliminar el punto al final de la cadena
        final errorMessage =
            '\n $errorCount. $description: "${error['bad']}"\n  • Sugerencias: ${error['better'].join(', ')}\n';

        // Incrementar el contador de errores
        errorCount++;

        return previousValue + errorMessage;
      });
    }
